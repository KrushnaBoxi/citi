...
public class CitiReportController {
	...
	@RequestMapping(value = ReportConstants.GET_REPORT_REQUEST_HANDLE_URL, method = RequestMethod.POST, produces = {
			"application/json" })
	...
	@RequestMapping(value = ReportConstants.SET_REPORT_REQUEST_HANDLE_URL, method = RequestMethod.POST, produces = {
			"application/json" })
	...
	@RequestMapping(value = ReportConstants.GET_PROGRAM_HANDLE_URL_PATH, method = RequestMethod.GET, produces = {
			"application/json" })
	public ResponseEntity<List<String>> getProgramList1(@PathVariable String fieldName) {
		List<String> responseMessage = new ArrayList<String>();
		System.out.println("field name : " + fieldName);
		if (fieldName.isEmpty()) {
			responseMessage.add("Field Missing");
